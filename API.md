...
&emsp;.svgRedirectFolder | string | URL folder to redirect any local disk SLD SVG paths. Default value is QGIS source folders on GitHub (not ideal as slow).
...
`qgisCompatibility.svgRedirectFolder` option: in QGIS if you use an SVG symbol that is not embedded the exported SLD will set the associated `OnlineResource` path to an absolute path on the local disk which cannot work for a web application (e.g. for system SLDs '/usr/share/qgis/svg' (Linux) or 'C:/OSGeo4W/apps/qgis/svg' (Windows)). This is also the case for any user or project SLDs even if "Save path: relative" is set in project properties. If `qgisCompatibility` is enabled, any such local SLD path references including '/svg/' will be redirected to `svgRedirectFolder` for which it is recommendend to make it a relative URL path (e.g. 'qgis_svg'). You would then need to host such a folder where you should copy just the required SVGs from the QGIS system/user/project folders. To allow for easy website development, `svgRedirectFolder` defaults to the QGIS Github versions (which should work for QGIS system SVGs), but this is not recommended for production as they might be slow to load (a console warning will be given).
