// ol-sld-styler SLD file load demo
//
// Style OpenLayers vector layers using SLD data, including styling data read
// from separate SLD file(s).
//
...
// individual SLD file(s) also loaded.
//
...
function loadAllSldFiles(sldFiles) {
    ...
    for (let sldFile of sldFiles) {
        const sldPromise = loadSld(sldFile)
            ...
            // Flag fatal error to user
            // (but code will actually limp on)
            .catch(error => fatalError(error));
    ...
    // Store SLD XML text string in sldsFromFiles object indexed by its
    // filename with folder path and .sld suffix stripped off
